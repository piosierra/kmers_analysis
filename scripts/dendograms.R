...
for (i in rownames(n)) {
  if (length(t[t$sample1==n[i,1],]$sample1) >0) {
    t[t$sample1==n[i,1],]$sample1 <- n[i,2]
  }
  if (length(t[t$sample2==n[i,1],]$sample2) >0) {
    t[t$sample2==n[i,1],]$sample2 <- n[i,2]
  }
}
# t <- read.table("~/rds/rds-durbin-group-8b3VcZwY7rY/projects/cichlid/pio/projects/20220201_kmers_analysis/results/15_genomes_k21")
...
# t[t$sample1=="Zea_mays.Zm-B73-REFERENCE-NAM-5.0.dna.toplevel",]$sample1 <- "Zea mays"
# t[t$sample2=="Zea_mays.Zm-B73-REFERENCE-NAM-5.0.dna.toplevel",]$sample2 <- "Zea mays"
# t[t$sample1=="Arabidopsis_thaliana.TAIR10.dna.toplevel",]$sample1 <- "Arabidopsis thaliana"
# t[t$sample2=="Arabidopsis_thaliana.TAIR10.dna.toplevel",]$sample2 <- "Arabidopsis thaliana"
...
# removing the most similar

# t <- t[t$sample1 != "Human" & t$sample2 != "Human",]
# t <- t[t$sample1 != "Pongo abelii" & t$sample2 != "Pongo abelii",]
# t <- t[t$sample1 != "Gorilla gorilla" & t$sample2 != "Gorilla gorilla",]
# t <- t[t$sample1 != "Pan troglodytes" & t$sample2 != "Pan troglodytes",]
